...
contract("DeCusSystem", (accounts) => {
  ...
  beforeEach(async () => {
    ...
    this.asset_meta = await AssetMeta.new([
      this.hbtc.address,
      this.wbtc.address,
    ]);
    ...
    await this.keeper_registry.setDependencies(this.asset_meta.address, {
      from: owner,
    });
    ...
    for (let i = 0; i < this.keepers.length; i++) {
      ...
      await this.keeper_registry.addKeeper(
        keeper,
        [this.wbtc.address],
        [keeperWbtcAmount],
        {
  ...
  it("role", async () => {
    expect(await this.decus_system.hasRole(DEFAULT_ADMIN_ROLE, owner)).to.be
      .true;

    expect(await this.keeper_registry.hasRole(DEFAULT_ADMIN_ROLE, owner)).to.be
      .true;

    expect(await this.keeper_registry.hasRole(KEEPER_ADMIN_ROLE, owner)).to.be
      .true;

    expect(await this.group_registry.hasRole(DEFAULT_ADMIN_ROLE, owner)).to.be
      .true;

    expect(
      await this.group_registry.hasRole(
        GROUP_ADMIN_ROLE,
        this.decus_system.address
      )
    ).to.be.true;

    expect(await this.group_registry.nGroups()).to.be.bignumber.equal(
      new BN(2)
    );
  });
  ...
  it("group id", async () => {
    expect(
      await this.group_registry.getGroupId(group0BtcAddress)
    ).to.be.bignumber.equal(group0Id);
    expect(
      await this.group_registry.getGroupId(group1BtcAddress)
    ).to.be.bignumber.equal(group1Id);
  });
  ...
  describe("overall state transition", () => {
    it("round", async () => {
      await this.decus_system.mintRequest(group1Id, group1BtcSatoshiAmount, {
        from: user1,
      });
      expect(
        await this.receipts.getWorkingReceiptId(group1Id)
      ).to.be.bignumber.equal(receipt1Id);
      expect(
        await this.receipts.getReceiptStatus(receipt1Id)
      ).to.be.bignumber.equal(new BN(1));

      await expectRevert(
        this.decus_system.mintRequest(group1Id, group1BtcSatoshiAmount, {
          from: user2,
        }),
      ...
      expect(
        await this.receipts.getReceiptStatus(receipt2Id)
      ).to.be.bignumber.equal(new BN(1));
      ...
      const ebtcBalance = group1BtcSatoshiAmount.mul(
        new BN(10).pow(new BN(10))
      );
      expect(await this.ebtc.balanceOf(recipient)).to.be.bignumber.equal(
        ebtcBalance
      );

      const amount = group1BtcSatoshiAmount.mul(new BN(10).pow(new BN(10)));
      await this.ebtc.approve(this.decus_system.address, amount, {
        from: user2,
      });

      await this.decus_system.burnRequest(receipt2Id, withdrawBtcAddress, {
        from: user2,
      });
      expect(
        await this.receipts.getReceiptStatus(receipt2Id)
    ...
    it("group mint start over", async () => {
      ...
      await this.ebtc.approve(this.decus_system.address, ebtcBalance, {
        from: user2,
      });
      await this.decus_system.burnRequest(receipt1Id, withdrawBtcAddress, {
        from: user2,
      });
      ...
      expect(
        await this.receipts.getWorkingReceiptId(group1Id)
      ).to.be.bignumber.equal(receipt2Id);
    });
  });

  describe("mintVerify", () => {
    beforeEach(async () => {
      await this.decus_system.mintRequest(group0Id, group0BtcSatoshiAmount, {
        from: user1,
      });
    });
    ...
    it("verify misorder", async () => {
      expect(
        await this.receipts.getWorkingReceiptId(group0Id)
      ).to.be.bignumber.equal(receipt1Id);
  ...
  describe("mint", () => {
    ...
    it("force mint", async () => {
      ...
      const rsp = await this.decus_system.forceMintRequest(
        group0Id,
        group0BtcSatoshiAmount,
        {
          from: user3,
      ...
      expect(
        await this.receipts.getWorkingReceiptId(group0Id)
      ).to.be.bignumber.equal(receiptId);
    });
    ...
    describe("ban", () => {
      ...
      it("should ban keeper", async () => {
        expect(
          await this.decus_system.cooldownUntil(this.keepers[0])
        ).to.be.bignumber.equal(new BN(0));
        ...
        expect(
          await this.decus_system.cooldownUntil(this.keepers[0])
        ).to.be.bignumber.equal(new BN(currentTimestamp + 60));
      });
    });
  });
});
