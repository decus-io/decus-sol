...
contract("CollateralLib", (accounts) => {
    ...
    describe("add keeper", () => {
        beforeEach(async () => {
            ...
            this.expectSatoshi = this.hbtcNumber.add(this.wbtcNumber).mul(this.satoshi_multiplier);
        ...
        it("delete", async () => {
            ...
            expect(await this.lib.exist(keeper1)).to.be.false;
            expect(await this.lib.getSatoshiValue(keeper1)).to.be.bignumber.equal(new BN(0));
        });
    });
});
