...
contract("GroupRegistry", (accounts) => {
  ...
  it("role", async () => {
    ...
    expect(await this.group_registry.hasRole(GROUP_ADMIN_ROLE, owner)).to.be
      .false;

    expect(await this.group_registry.hasRole(GROUP_ADMIN_ROLE, decusSystem)).to
      .be.true;
  });
  ...
  describe("add", () => {
    ...
    beforeEach(async () => {
      await this.hbtc.approve(this.keeper_registry.address, amount, {
        from: keeper1,
      });
    ...
    it("add", async () => {
      ...
      expect(
        await this.group_registry.getGroupId(btcAddress)
      ).to.be.bignumber.equal(groupId);
      expect(await this.group_registry.exist(groupId)).to.be.true;
      expect(
        await this.group_registry.getGroupAllowance(groupId)
      ).to.be.bignumber.equal(amount);

      const keeperGroupIdArray = await this.group_registry.getKeeperGroupIds(
        keeper1
      );
    ...
    it("reject dup id", async () => {
      await this.group_registry.addGroup(
        required,
        amount,
        btcAddress,
        this.keepers,
    ...
    describe("delete", () => {
      ...
      it("delete success", async () => {
        ...
        const rsp = await this.group_registry.deleteGroup(id, {
          from: decusSystem,
        });
