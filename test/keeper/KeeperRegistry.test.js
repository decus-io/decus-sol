...
contract("KeeperRegistry", (accounts) => {
  ...
  beforeEach(async () => {
    ...
    this.assetMeta = await AssetMeta.new([
      this.hbtc.address,
      this.wbtc.address,
    ]);
  ...
  it("role", async () => {
    expect(await this.keeperRegistry.hasRole(DEFAULT_ADMIN_ROLE, owner)).to.be
      .true;

    expect(await this.keeperRegistry.hasRole(KEEPER_ADMIN_ROLE, owner)).to.be
      .false;

    expect(await this.keeperRegistry.hasRole(KEEPER_ADMIN_ROLE, keeperAdmin)).to
      .be.true;
  });
  ...
  describe("one keeper", () => {
    ...
    beforeEach(async () => {
      await this.keeperRegistry.setDependencies(this.assetMeta.address, {
        from: owner,
      });
      await this.hbtc.approve(this.keeperRegistry.address, amount, {
        from: user1,
      });
    });
    ...
    describe("remove", () => {
      beforeEach(async () => {
        await this.keeperRegistry.addKeeper(
          user1,
          [this.hbtc.address],
          [amount],
          {
  ...
  describe("import keepers", () => {
    ...
    it("import", async () => {
      ...
      await this.hbtc.approve(this.keeperRegistry.address, allowance, {
        from: auction,
      });
      await this.wbtc.approve(this.keeperRegistry.address, allowance, {
        from: auction,
      });
