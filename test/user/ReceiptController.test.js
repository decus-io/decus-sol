...
contract("ReceiptController", (accounts) => {
  ...
  it("role", async () => {
    ...
    expect(await this.controller.hasRole(RECEIPT_ADMIN_ROLE, user1)).to.be
      .false;
  });
  ...
  it("request deposit", async () => {
    ...
    expect(
      await this.controller.getReceiptStatus(receiptId)
    ).to.be.bignumber.equal(this.receiptId);
    expect(await this.controller.getUserAddress(receiptId)).to.equal(user1);
  });

  it("request deposit fail", async () => {
    await expectRevert(
      this.controller.depositRequest(user1, this.groupId, this.amount, {
        from: user1,
      }),
      "require admin role"
    );
  ...
  describe("state", () => {
    ...
    it("deposit received", async () => {
      await this.controller.depositReceived(
        this.receiptId,
        this.txId,
        this.height,
        {
      ...
      expect(
        await this.controller.getReceiptStatus(this.receiptId)
    ...
    it("withdraw requested", async () => {
      ...
      await this.controller.withdrawRequest(this.receiptId, this.btcAddress, {
        from: owner,
      });
